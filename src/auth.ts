...
import { config, type AuthTokens } from './config.js';
 ...
 */
async function registerWithLimitOrders(tokens: AuthTokens): Promise<void> {
...
export async function authenticate(): Promise<AuthTokens | null> {
  ...
  try {
    ...
    const response = await axios.post<{ data: AuthResponse }>(
      `${authUrl}/user/auth/authenticate`,
      {
        auth_method: 'agent',
