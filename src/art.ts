...
export const bobaGradient = (text: string): string => bobaGrad(text);
export const successGradient = (text: string): string => successGrad(text);
export const errorGradient = (text: string): string => errorGrad(text);
...
function colorizeFrame(frame: string[]): string {
...
export async function bouncyLoader(message: string, duration: number = 3000): Promise<void> {
...
export async function bubbleLoader(message: string, duration: number = 1500): Promise<void> {
...
export async function spinnerLoader(
  message: string,
  style: keyof typeof spinners = 'dots'
): Promise<{ stop: () => void; succeed: (text?: string) => void; fail: (text?: string) => void }> {
  const spinner = createSpinner(message, style);
  spinner.start();

  return {
    stop: () => spinner.stop(),
    succeed: (text?: string) => spinner.succeed(text ? matcha(text) : undefined),
    fail: (text?: string) => spinner.fail(text ? chalk.hex('#FF6B6B')(text) : undefined),
  };
}
...
export function createSpinner(text: string, style: keyof typeof spinners = 'dots'): Ora {
