...
function formatUSD(value: number | string | undefined): string {
  if (value === undefined || value === null) return matchaDim('--');
  const num = typeof value === 'string' ? parseFloat(value) : value;
  if (isNaN(num)) return matchaDim('--');
  if (num >= 1_000_000_000) return gold(`$${(num / 1_000_000_000).toFixed(2)}B`);
  if (num >= 1_000_000) return gold(`$${(num / 1_000_000).toFixed(2)}M`);
  if (num >= 1_000) return gold(`$${(num / 1_000).toFixed(2)}K`);
  if (num >= 1) return gold(`$${num.toFixed(2)}`);
...
function truncateAddress(addr: string | undefined): string {
  if (!addr) return matchaDim('--');
  return matchaDim(`${addr.slice(0, 6)}...${addr.slice(-4)}`);
}

// Format portfolio response
export function formatPortfolio(data: any): string {
  const lines: string[] = [];
  ...
  if (tokens.length > 0) {
    ...
    tokens.slice(0, 8).forEach((token: any) => {
...
export function formatTokenSearch(data: any): string {
  ...
  tokens.slice(0, 15).forEach((t: any) => {
    table.push([
      matcha(t.symbol || '???'),
...
// Format token info
export function formatTokenInfo(data: any): string {
  const lines: string[] = [];
  ...
  if (data.audit || data.security) {
    ...
    const checkMark = (val: boolean | undefined) => val ? matcha('✓') : red('✗');
...
// Format PnL chart
export function formatPnLChart(data: any): string {
  const lines: string[] = [];
  ...
  const values = chartData.map((p: any) => {
  ...
  try {
    const chart = asciichart.plot(values, {
      ...
      format: (x: number) => formatUSD(x).padStart(10),
    });
    lines.push(chart);
...
// Format swap quote
export function formatSwapQuote(data: any): string {
  const lines: string[] = [];
...
export function formatTradeResult(data: any): string {
  const lines: string[] = [];
...
export function formatTrendingTokens(data: any): string {
...
export function formatTokenChart(data: any): string {
  const lines: string[] = [];
  ...
  const values = chartData.map((candle: any) => {
  ...
  try {
    const chart = asciichart.plot(values, {
      ...
      colors: [asciichart.magenta],
      format: (x: number) => {
        if (x >= 1) return ('$' + x.toFixed(2)).padStart(12);
        if (x >= 0.01) return ('$' + x.toFixed(4)).padStart(12);
...
export function formatToolResult(tool: string, data: any): string | null {
  try {
    // Parse if string
