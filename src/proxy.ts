...
function createToolSpinner(tool: string, action: string): Ora {
  return ora({
...
function formatResult(tool: string, result: any): string {
  try {
...
export function createProxyServer(): ProxyServer {
  ...
  app.post('/call', async (req, res) => {
    ...
    const isFakeId = (id: string) => {
      ...
      if (/^1+$/.test(id)) return true; // "111111..."
      if (/^0+$/.test(id)) return true; // "000000..."
      if (id === 'me' || id === 'self') return true;
      // Wallet addresses (Claude sometimes confuses wallet with user_id)
      if (id.startsWith('0x') && id.length === 42) return true; // EVM address
      if (id.length >= 32 && id.length <= 44 && /^[1-9A-HJ-NP-Za-km-z]+$/.test(id)) return true; // Solana address (base58)
      return false;
    };

    // Helper to detect Solana chain
    const isSolanaChain = (chain: any) => {
      if (!chain) return false;
      if (chain === 1399811149) return true;
    ...
    if (args) {
      ...
      if (args.solana_address === 'my-wallet-svm' || args.solana_address === 'me' || args.solana_address === 'self') {
        if (solanaAddress) args.solana_address = solanaAddress;
      }
      ...
      if (swapExecutionTools.includes(tool)) {
        ...
        if (!args.taker || args.taker === 'my-wallet-evm' || args.taker === 'my-wallet-svm') {
          if (isSolanaChain(chain)) {
            if (solanaAddress) args.taker = solanaAddress;
          } else if (evmAddress) {
        ...
        if (!args.from_address && !args.fromAddress) {
          if (isSolanaChain(chain)) {
            if (solanaAddress) args.from_address = solanaAddress;
          } else if (evmAddress) {
